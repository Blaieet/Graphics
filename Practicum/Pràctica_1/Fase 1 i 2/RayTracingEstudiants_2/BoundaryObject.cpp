...
{ 
    ...
    for (int i = 0; i < cares.size(); i++) {
        ...
        Triangle *tri = new Triangle(vec3(v1.x, v1.y, v1.z), vec3(v2.x, v2.y, v2.z), vec3(v3.x, v3.y, v3.z), m);
...
void BoundaryObject::aplicaTG(TG *t) {
    ...
    if (dynamic_cast<Scale *>(t)) {
        for (int i = 0; i < triangles.size(); i++) {
            triangles[i]->aplicaTGObjecte(t,this->centre);
        }
...
bool BoundaryObject::hit(const Ray& r, float t_min, float t_max, HitInfo& rec) const {
    ...
    for (int i = 0; i < numTriangles; i++) {
        ...
        triangles[i]->hit(r,t_min,t_max,rec);
