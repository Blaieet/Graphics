...
void Object::draw(){
    ...
    //text
    glBufferSubData( GL_ARRAY_BUFFER, sizeof(point4)*Index+sizeof(point4)*Index, sizeof(vec2)*Index, vCoords );
    ...
    for(unsigned int i=0; i<cares.size(); i++){
        for(unsigned int j=0; j<cares[i].idxVertices.size(); j++){
            ...
            if (!textVertexs.empty()) {
                vCoords[Index] = textVertexs[cares[i].idxTextures[j]];
...
void Object::initTexture()
 {
    ...
    //setTexture(new QOpenGLTexture(QImage("://resources/textures/EyesWhite.jpg")));
...
Capsa3D Object::calculCapsa3D()
{
    ...
    this->centre = vec3(capsa.a/2.0 + capsa.pmin.x, capsa.h/2.0 + capsa.pmin.y, capsa.p/2.0 + pmin.z);
